...
- (void)viewDidLoad {
	...
	if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) { // iPad.
		...
		progressBarVOffset=185; //set y position of the progress bar
		removeButtonHOffset=120; // set x position of btn remove 
		removeButtonVOffset=190; //set y position of btn remove 
		openButtonHOffset=120; //set x position of btn remove
		openButtonVOffset=130; // set y position of btn Open/Downlaod
	...
	if (!fileAlreadyExists) {
        ...
        if ([fileManager fileExistsAtPath:[pdfPathTempForResume stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.pdf",page]]]) {
			...
			[aButton setImage:[UIImage imageWithContentsOfFile:MF_BUNDLED_RESOURCE(@"FPKKioskBundle",@"RESUME1",@"png")] forState:UIControlStateNormal];
            ...
            [aButton setImage:[UIImage imageWithContentsOfFile:MF_BUNDLED_RESOURCE(@"FPKKioskBundle",@"Download1",@"png")] forState:UIControlStateNormal];
        }
        [aButton addTarget:self action:@selector(actionDownloadPdf:) forControlEvents:UIControlEventTouchUpInside];
	} else {
		...
		[aButton setImage:[UIImage imageWithContentsOfFile:MF_BUNDLED_RESOURCE(@"FPKKioskBundle",@"View1",@"png")] forState:UIControlStateNormal];
		[aButton addTarget:self action:@selector(actionOpenPdf:) forControlEvents:UIControlEventTouchUpInside];
	}
	...
	[aButton setImage:[UIImage imageWithContentsOfFile:MF_BUNDLED_RESOURCE(@"FPKKioskBundle",@"REmove1",@"png")] forState:UIControlStateNormal];
	[aButton setTag:documentNumber];
	[aButton setAutoresizingMask:UIViewAutoresizingFlexibleBottomMargin|UIViewAutoresizingFlexibleRightMargin];
	...
	if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
		...
		aLabel = [[UILabel alloc ] initWithFrame:CGRectMake(0, 55, 400, 30) ];
...
-(void)requestStarted:(ASIHTTPRequest *)request{
	...
	[aButton setImage:[UIImage imageWithContentsOfFile:MF_BUNDLED_RESOURCE(@"FPKKioskBundle",@"Pause1",@"png")] forState:UIControlStateNormal];
